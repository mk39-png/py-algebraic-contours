...
def main(args: argparse.Namespace) -> None:
    ...
    """
    mesh_filename = pathlib.Path(args.input)
    camera_filename = pathlib.Path(args.camera)
    ...
    FT: MatrixXi
    FN: MatrixXi
    initial_V, uv, N, F, FT, FN = igl.readOBJ(mesh_filename)
    ...
    with open(output_dir / "per_view.csv", "a", encoding="utf-8") as out_per_view:
        out_per_view.write(f"{mesh_filename},"
                           f"{camera_filename},"
                           f"[{rotation_frame[0, :]}] "
                           f"[{rotation_frame[1, :]}] "
                           f"[{rotation_frame[2, :]}],"
                           ...
                           f"{initial_contour_network_time},"
                           f"{contour_network.surface_update_position_time},"
                           f"{contour_network.compute_contour_time},"
                           f"{contour_network.compute_cusp_time},"
                           f"{contour_network.compute_intersection_time},"
                           f"{contour_network.compute_visibility_time},"
                           f"{contour_network.compute_projected_time},"

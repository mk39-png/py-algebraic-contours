...
def test_init_chain_start_nodes(testing_fileinfo) -> None:
    """
    ...
    param_filepath: pathlib.Path = base_data_folderpath / "contour_network" / \
        "projected_curve_network" / "init_projected_curve_network"
    ...
    projected_curve_network_test = ProjectedCurveNetwork(
        ...
        deserialize_list_list_varying_lengths(
            param_filepath / "chains.csv"),
        deserialize_eigen_matrix_csv_to_numpy(
            param_filepath / "chain_labels.csv").tolist(),
        deserialize_list_list_varying_lengths_float(
            param_filepath / "interior_cusps.csv"),
        deserialize_eigen_matrix_csv_to_numpy(
            param_filepath / "has_cusp_at_base.csv").astype(bool).tolist(),
        deserialize_list_list_varying_lengths_float(
            param_filepath / "intersections.csv"),
        deserialize_list_list_varying_lengths(
            param_filepath / "intersection_indices.csv"),
        deserialize_list_list_intersection_data(
            param_filepath / "intersection_data.json"),
        num_intersections=176)

    # Compare results
    result_filepath: pathlib.Path = base_data_folderpath / "contour_network" / \
        "projected_curve_network" / "init_chain_start_nodes"
    compare_list_node_geometry(result_filepath / "nodes_out.json",
                               projected_curve_network_test.nodes)
    compare_eigen_numpy_matrix(result_filepath / "chain_start_nodes.csv",
                               np.array(projected_curve_network_test.chain_start_nodes, dtype=int))


def test_split_segments_at_cusps(testing_fileinfo) -> None:
    """
    ...
    filepath: pathlib.Path = base_data_folderpath / "contour_network" / \
        "projected_curve_network" / "split_segments_at_cusps"
    ...
    compare_list_segment_geometry(filepath / "segments_out.json",
                                  segments_test)
    compare_list_node_geometry(filepath / "nodes_out.json",
                               nodes_test)


def test_connect_segment_intersections(testing_fileinfo) -> None:
    """
    ...
    base_data_folderpath, _ = testing_fileinfo
    filepath: pathlib.Path = base_data_folderpath / "contour_network" / \
        "projected_curve_network" / "connect_segment_intersections"
...
def test_split_segments_at_intersections(testing_fileinfo) -> None:
    """
    ...
    base_data_folderpath, _ = testing_fileinfo
    filepath: pathlib.Path = base_data_folderpath / "contour_network" / \
        "projected_curve_network" / "split_segments_at_intersections"
    ...
    compare_list_list_varying_lengths(filepath / "intersection_nodes.csv",
                                      intersection_nodes)
    compare_list_list_varying_lengths(filepath / "split_segment_indices.csv",
                                      split_segment_indices)
...
def test_remove_redundant_intersections(testing_fileinfo) -> None:
    """
    ...
    base_data_folderpath, _ = testing_fileinfo
    filepath: pathlib.Path = base_data_folderpath / "contour_network" / \
        "projected_curve_network" / "remove_redundant_intersections"
    ...
    # Compare results
    compare_list_list_intersection_data_from_file(
        filepath / "intersection_data_out.json", intersection_data_test)


def test_mark_open_chain_endpoints(testing_fileinfo) -> None:
    """
    ...
    base_data_folderpath, _ = testing_fileinfo
    filepath: pathlib.Path = base_data_folderpath / "contour_network" / \
        "projected_curve_network" / "mark_open_chain_endpoints"
    ...
    chains: list[list[SegmentIndex]] = deserialize_list_list_varying_lengths(
        filepath / "chains.csv")
    nodes_in: list[NodeGeometry] = _deserialize_list_node_geometry(
        filepath / "nodes_in.json")
...
def test_build_projected_curve_network_without_intersections(testing_fileinfo) -> None:
    """

...
def test_read_write_spline_surface_serialization(testing_fileinfo) -> None:
    ...
    filepath_test: pathlib.Path = base_data_folderpath / filename_test

    # NOTE: need a placeholder to call write_spline() and deserialize()...
    # TODO: better to separate deserialize() and write_spline() as separate from,
    # QuadraticSplineSurface class and take QuadraticSplineSurface as the parameter or whatnot.
    spline_surface_placeholder: QuadraticSplineSurface = QuadraticSplineSurface.from_file(
        filepath=filepath_control)

    # Write the saved spline data to an external file.
    # So, converting Eigen TXT -> Numpy Implementation -> NumPy TXT
...
# def test_read_view_spline_surface_deserialization(testing_fileinfo) -> None:
#     """
...
#     spline_surface: QuadraticSplineSurface = QuadraticSplineSurface.from_file(filepath=obj_filepath)
#     spline_surface.view()

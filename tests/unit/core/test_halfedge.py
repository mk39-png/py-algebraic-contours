...
from pyalgcon.core.common import (Index, MatrixNx3i, MatrixXf, MatrixXi,
...
def test_halfedge_spot_mesh(testing_fileinfo,
    ...
    filepath: pathlib.Path = base_data_folderpath / "core" / "halfedge"
    V: np.ndarray
    uv: np.ndarray
    F: np.ndarray
    FT: np.ndarray
    ...
    compare_eigen_numpy_matrix(filepath / "e2he.csv",
                               np.array(halfedge.edge_to_halfedge_map))
    compare_eigen_numpy_matrix(filepath / "f2he.csv",
                               np.array(halfedge.f2he))
    compare_eigen_numpy_matrix(filepath / "face.csv",
                               np.array(halfedge.face))
    compare_eigen_numpy_matrix(filepath / "from.csv",
                               np.array(halfedge._from))
    compare_eigen_numpy_matrix(filepath / "he2e.csv",
                               np.array(halfedge.halfedge_to_edge_map))
    compare_eigen_numpy_matrix(filepath / "next.csv",
                               np.array(halfedge.next))
    compare_eigen_numpy_matrix(filepath / "opp.csv",
                               np.array(halfedge.opp))
    compare_eigen_numpy_matrix(filepath / "out.csv",
                               np.array(halfedge.out))
    compare_eigen_numpy_matrix(filepath / "to.csv",
